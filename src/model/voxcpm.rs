...
impl SinusoidalPosEmb {
    pub fn new(dim: usize) -> Result<Self> {
        anyhow::ensure!(
            ...
            "SinusoidalPosEmb requires dim to be even"
        );
        Ok(Self { dim })
    }
...
impl VoxCPMLocDiT {
    pub fn new(vb: VarBuilder, config: VoxMiniCPM4Config, in_channels: usize) -> Result<Self> {
        ...
        anyhow::ensure!(config.vocab_size == 0, "vocab_size must be 0 for local DiT");
...
impl UnifiedCFM {
    ...
    pub fn forward(
        ...
        let t_span = t_span
            ...
            .affine(1.0, -1.0)?
            .add(&t_span)?
            .affine(config.sway_sampling_coef, 0.0)?
            .add(&t_span)?;
    ...
    pub fn solve_euler(
        ...
        let mut sol = Vec::with_capacity(t_span_len);
        ...
        for step in 1..t_span_len {
            if use_cfg_zero_star && step <= zero_init_steps {
                ...
                if use_cfg_zero_star {
                    ...
                    let mut shape: Vec<usize> = vec![b; 1 + dphi_dt.rank() - 1];
                    shape[1..].fill(1);
                    st_star = st_star.reshape(shape)?;
                }
...
impl VoxCPMModel {
    ...
    fn encode_text_tokens(&self, text: &str) -> Result<Tensor> {
    ...
    fn encode_audio_feat(&self, audio_path: &str) -> Result<Tensor> {
        ...
        tracing::info!(
            "encode_audio_feat: after vae encode, feat shape={:?}, mean={:?}, min={:?}, max={:?}",
            feat.dims(),
            feat.mean_all(),
            feat.min_all(),
            feat.max_all()
        ...
        let feat = feat
            ...
            .permute((1, 2, 0))?;
        tracing::info!(
            "encode_audio_feat: after reshape/permute, feat shape={:?}, mean={:?}",
            feat.dims(),
            feat.mean_all()
        );
        Ok(feat)
    ...
    pub fn generate(
        ...
        let max_len = config
            .max_len
            .min((target_text_length as f64 * config.retry_ratio_threshold + 10.0) as usize);
    ...
    fn _generate(
        ...
        let latent_pred =
            self.inference(&text_token, &text_mask, &audio_feat, &audio_mask, config)?;
    ...
    fn encode_prefix(
        ...
        let embed_tokens = self
            ...
            .embed_tokens
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("embed_tokens not initialized"))?;
        let text_embed = embed_tokens.forward(text)?.affine(scale_emb as f64, 0.0)?;
        tracing::info!(
            "encode_prefix: text_embed {:?}, feat_embed {:?}, text_mask {:?}, feat_mask {:?}",
            text_embed.dims(),
            feat_embed.dims(),
            text_mask.dims(),
            feat_mask.dims()
        ...
        tracing::info!(
            "encode_prefix stats: text_mask sum={:?}, feat_mask sum={:?}, text_embed mean={:?}, feat_embed mean={:?}",
            text_mask.sum_all(),
            feat_mask.sum_all(),
            text_embed.mean_all(),
            feat_embed.mean_all(),
        ...
        let enc_outputs = self.base_lm.forward_with_cache(&combined_embed, 0)?;
        ...
        let feat_weighted = feat_mask.unsqueeze(D::Minus1)?.broadcast_mul(feat_embed)?;
        let concat = Tensor::cat(&[&enc_outputs, &feat_weighted], D::Minus1)?;
        let input_embeds = self.fusion_concat_proj.forward(&concat)?;
        let residual_enc_outputs = self.residual_lm.forward_with_cache(&input_embeds, 0)?;

        Ok((enc_outputs, residual_enc_outputs))
    }

    fn autoregressive_loop(
        &mut self,
        feat: &Tensor,
        enc_outputs: &Tensor,
        residual_enc_outputs: &Tensor,
        t: usize,
        config: &InferenceConfig,
    ) -> Result<Tensor> {
        let mut prefix_feat_cond = feat.i((.., t - 1, ..))?;
        let mut pred_feat_seq = Vec::with_capacity(config.max_len);
        let mut position_id = 0;
        let mut seq_len = t;
        let mut lm_hidden = enc_outputs.i((.., t - 1, ..))?;
        let mut residual_hidden = residual_enc_outputs.i((.., t - 1, ..))?;

        tracing::info!("autoregressive loop: t={}, max_len={}", t, config.max_len);
        let loop_start = std::time::Instant::now();
        for i in 0..config.max_len {
            let dit_hidden_1 = self.lm_to_dit_proj.forward(&lm_hidden)?;
            let dit_hidden_2 = self.res_to_dit_proj.forward(&residual_hidden)?;
            let dit_hidden = Tensor::cat(&[&dit_hidden_1, &dit_hidden_2], D::Minus1)?;
            ...
            let pred_feat = self
                ...
                .transpose(1, 2)?;
            let curr_embed = self.feat_encoder.forward(&pred_feat.unsqueeze(1)?)?;
            ...
            let stop_flag = self
                ...
                .i(0)?
                .to_scalar::<u32>()?;
            if i > config.min_len && stop_flag == 1 {
                tracing::info!(
                    "loop done at step {}/{}, {:.1}s",
                    i,
                    config.max_len,
            ...
            let curr_embed_0 = curr_embed.i((.., 0, ..))?;
            let concat = Tensor::cat(&[&lm_hidden, &curr_embed_0], D::Minus1)?;
            let curr_residual_input = self.fusion_concat_proj.forward(&concat)?;
        ...
        let pred_seq = Tensor::cat(&pred_feat_seq, 1)?;
    ...
    fn inference(
        ...
        tracing::info!(
            "inference: after feat_encoder, feat_embed mean={:?}",
            feat_embed.mean_all()
        );
        let feat_embed = self.enc_to_lm_proj.forward(&feat_embed)?;
        tracing::info!(
            "inference: after enc_to_lm_proj, feat_embed mean={:?}",
            feat_embed.mean_all()
        );

        let (enc_outputs, residual_enc_outputs) =
            self.encode_prefix(text, text_mask, &feat_embed, feat_mask)?;

        self.autoregressive_loop(feat, &enc_outputs, &residual_enc_outputs, t, config)
    }
